...
Q11:
req.query
GET /search?name=John&age=22
app.get("/search", (req, res) => {
  console.log(req.query.name); // John
  console.log(req.query.age);  // 22
});
req.params
GET /user/101
...
app.post("/login", (req, res) => {
  console.log(req.body.email);    // a@gmail.com
  console.log(req.body.password); // 1234
});

Q12:
Storing passwords directly is dangerous because If database is hacked, passwords are exposed , Users often reuse passwords, Violates security standards so Converts password into irreversible encrypted value, Same password → same hash , Original password cannot be retrieved

Q13:Restricts number of requests a user can make in a given time
Prevents brute-force attacks , Protects against DDoS attacks, Improves server performance , Prevents API abuse
...
Q15:
findOne()
User.findOne({ email: "a@gmail.com" });
Reasult - { name: "A", email: "a@gmail.com" }
find()
